...
function completarFila(element,index,arr) {
    ...
    <td>
    <a href='actualizar_usuario.html?id=${element.id_usuario}' class='btn btn-warning btn-sm'>Actualizar</a>
    <a href='eliminar_usuario.html?id=${element.id_usuario}' class='btn btn-danger btn-sm'>Eliminar</a>
    </td>
    </tr>`
}
...
function agregarUsuario() {
    //Obtenemos todos los campos necesarios para agregar un usuario.
    ...
    var clave = document.getElementById("txt_password");
    ...
    // Username, no vacío.
    if (user_name.value === "") {
        marcarInvalido(user_name);
        existenErrores = true;
    } else {
        marcarValido(user_name);
    }

    // Password,  más de 3 caracteres.
    if (clave.value.length < 3) {
        marcarInvalido(clave);
        existenErrores = true;
    } else {
        marcarValido(clave);
    }

    // Mostrar alerta si hay existen errores al ingresar los datos del usuario.
    if (existenErrores) {
    ...
    fetch("http://144.126.136.43/api/usuario?_size=200", requestOptions)
    .then((response) => {
...
//Funcion que hace visible la contraseña al momento de pulsar el ojo.
function passVisibleAgregar() {
...
function actualizarUsuario() {
    ...
    //Alerta para comprobar que los campos se ingresaron correctamente.
    var alertaActualizacion = document.getElementById("alertaUsuarioActualizacion");
    ...
    // Username, no vacío.
    if (nuevo_username.value === "") {
        marcarInvalido(nuevo_username);
        existenErrores = true;
    } else {
        marcarValido(nuevo_username);
    }

    // Password,  más de 3 caracteres.
    if (nueva_clave.value.length < 3) {
        marcarInvalido(nueva_clave);
        existenErrores = true;
    } else {
        marcarValido(nueva_clave);
    }
...
function obtenerIdActualizacion() {
    ...
    // Invocamos un método para obtener los datos del usuario en particular.
...
function completarFormulario(element,index,arr) {
    var [nombre1Usuario, nombre2Usuario = ""] = element.nombres.split(" ");
    var [apellido1Usuario, apellido2Usuario = ""] = element.apellidos.split(" ");
    var emailUsuario = element.email;
    var celularUsuario = element.celular;
    var usernameUsuario = element.username;
    var claveUsuario = element.password;

    document.getElementById("txt_nuevo_nombre_1").value = nombre1Usuario;
    document.getElementById("txt_nuevo_nombre_2").value = nombre2Usuario;
    document.getElementById("txt_nuevo_apellido_1").value = apellido1Usuario;
    document.getElementById("txt_nuevo_apellido_2").value = apellido2Usuario;
    document.getElementById("txt_nuevo_email").value = emailUsuario;
    document.getElementById("txt_nuevo_celular").value = celularUsuario;
    document.getElementById("txt_nuevo_username").value = usernameUsuario;
    document.getElementById("txt_nuevo_password").value = claveUsuario;
}

//Función que hace visible la contraseña al apretar el ojo.
function passVisibleActualizar() {
    var pass = document.getElementById("txt_nuevo_password");
    var icono = document.getElementById("icono-ojo");
...
function eliminarUsuario() {
    ...
    .then((response) => {
        if (response.status == 200) {
            ...
            var alertaEliminacion = document.getElementById("alertaUsuarioEliminacion");
            alertaEliminacion.textContent = "El usuario que desea eliminar se encuentra en uso.";
